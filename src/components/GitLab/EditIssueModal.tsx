...
export function EditIssueModal({ open, issue, repoPath, onClose, onUpdated }: EditIssueModalProps) {
  ...
  async function handleSave() {
    if (!title.trim()) {
      showToast(t('gitlab.issue.titleRequired'), 'error')
      return
    }
    setBusy(true)
    try {
      await window.gitfreddo.gitlabUpdateIssue(repoPath, issue.number, {
        title: title.trim(),
        body
      })
      await onUpdated()
      showToast(t('gitlab.issue.updated'), 'success')
      onClose()
    } catch (error) {
  ...
  return (
    <Modal open={open} title={t('gitlab.issue.editTitle', { number: issue.number })} onClose={onClose}>
