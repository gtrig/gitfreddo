...
const issue: BitbucketIssue = {
  ...
  title: 'Broken build',
  state: 'open',
  htmlUrl: 'https://bitbucket.org/acme/app/issues/7',
  user: 'dev',
...
describe('Bitbucket EditIssueModal', () => {
  afterEach(() => cleanup())
  ...
  it('requires a non-empty title before saving', async () => {
    ...
    expect(window.gitfreddo.bitbucketUpdateIssue).not.toHaveBeenCalled()
  })
  ...
  it('updates the issue and closes on success', async () => {
    ...
    await waitFor(() => {
      expect(window.gitfreddo.bitbucketUpdateIssue).toHaveBeenCalledWith('/tmp/repo', 7, {
        title: 'Fixed build',
        body: 'CI fails on main'
      })
    })
  ...
  it('shows API errors from update', async () => {
    vi.mocked(window.gitfreddo.bitbucketUpdateIssue).mockRejectedValue(new Error('Not allowed'))
