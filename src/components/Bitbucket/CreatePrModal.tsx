...
export function CreatePrModal({
  ...
  return (
    <Modal open={open} title={t('bitbucket.pr.createTitle')} onClose={onClose}>
      <div className="space-y-3 p-4">
            ...
            {aiEnabled && (
              <div className="absolute right-2 top-1/2 -translate-y-1/2">
                <AiActionButton
                  ...
                  aria-label={t('bitbucket.pr.fillWithAi')}
                  title={t('bitbucket.pr.fillWithAi')}
                />
              </div>
            )}
          </div>
        ...
        <label className="block text-sm">
          <span className="text-gf-fg-muted">{t('bitbucket.pr.description')}</span>
          <textarea
            value={body}
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="block text-sm">
            <span className="text-gf-fg-muted">{t('bitbucket.pr.head')}</span>
            <Select
              value={head}
          ...
          <label className="block text-sm">
            <span className="text-gf-fg-muted">{t('bitbucket.pr.base')}</span>
            <Select
              value={base}
...
export function MergePrButton({
  ...
  async function merge(method: BitbucketMergeMethod) {
    setBusy(true)
    try {
