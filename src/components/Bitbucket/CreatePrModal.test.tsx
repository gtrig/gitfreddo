...
describe('Bitbucket CreatePrModal', () => {
  ...
  it('lists local branches in head and base dropdowns', async () => {
    ...
    expect(screen.getByLabelText(/^head$/i)).toHaveValue('feature')
  ...
  it('includes preferred branches that are not in the branch list', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText(/^head$/i).querySelector('option[value="orphan-head"]')).toBeTruthy()
    })
    expect(screen.getByLabelText(/^base$/i).querySelector('option[value="orphan-base"]')).toBeTruthy()
  })
