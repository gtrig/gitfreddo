...
export function EditIssueModal({ open, issue, repoPath, onClose, onUpdated }: EditIssueModalProps) {
  ...
  async function handleSave() {
    if (!title.trim()) {
      showToast(t('github.issue.titleRequired'), 'error')
      return
    }
    setBusy(true)
    try {
      await window.gitfreddo.githubUpdateIssue(repoPath, issue.number, {
        title: title.trim(),
        body
      })
      await onUpdated()
      showToast(t('github.issue.updated'), 'success')
      onClose()
    } catch (error) {
  ...
  return (
    <Modal open={open} title={t('github.issue.editTitle', { number: issue.number })} onClose={onClose}>
