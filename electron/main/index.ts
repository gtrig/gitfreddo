...
function registerIpc(): void {
  ...
  ipcMain.handle('gitfreddo:switch-workspace', async (_event, repoPath: string) => {
    const normalized = await repoManager.switchRepo(repoPath)
    repoWatcherManager.watch(normalized)
    return normalized
  })
  ...
  ipcMain.handle(
    ...
    async (_event, method: string, params?: unknown, repoPath?: string) => {
      if (method === 'ai.fill') {
        ...
        const enriched = await enrichAiContext(repoManager, params as AiFillParams, repoPath)
  ...
  ipcMain.handle('gitfreddo:ai-fill', async (_event, params: AiFillParams, repoPath?: string) => {
    ...
    const enriched = await enrichAiContext(repoManager, params, repoPath)
    return aiFill(aiConfigFromSettings(settings), enriched)
  })

  const settingsRef = {
    get: () => settings,
    set: (next: AppSettings) => { settings = next }
  }
