...
- **What:** Successful `commit.create` closes the working/staged diff overlay; commit, stash, and commit-range previews stay open. Failed commits and other mutations (e.g. stash) leave the preview as-is. Tests cover close modes, leave-open modes, and the commit-only method guard.
